...
neigb = [ -1 0;
 ...
 while (pixdist < 0.08 && reg_size < numel(I))
     %增加新的邻域像素到neg_list中
     for j=1:4
         xn = x + neigb(j,1);
         yn = y + neigb(j,2);
...
%     end
 ...
 hold off;
 subplot(2,2,2),imshow(J),title('区域增长');
 J = bwmorph(J,'dilate');%补充空洞
 J = bwmorph(J,'dilate');
 subplot(2,2,3),imshow(J),title('空洞填充');
 subplot(2,2,4),imshow(I+J),title('分割结果');