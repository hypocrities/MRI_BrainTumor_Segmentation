...
class Prediction(object):
    ...
    def predict_volume(self, filepath_image,show):
        ...
        gt = glob( filepath_image + '/*O.OT.*.nii.gz')
        t1s = glob( filepath_image + '/*_T1.*.nii.gz')
        t1c = glob( filepath_image + '/*_T1c.*.nii.gz')
...
if __name__ == "__main__":
    ...
    # path_HGG = glob('NIFTI/HGG/**')
    # path_LGG = glob('NIFTI/LGG/**')
    #
    ...
    #input type list
    path = argv[1]
    brain_seg_pred.predict_multiple_volumes([path], save=True,show=True)
